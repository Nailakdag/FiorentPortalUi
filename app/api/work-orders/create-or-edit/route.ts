import {
  DELETE_WORK_ORDERS_DELETE,
  GET_WORK_ORDER_FOR_EDIT,
  POST_WORK_ORDERS_CREATE_OR_EDIT,
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data, error } = await fetchFromFiorentApi(
      `${GET_WORK_ORDER_FOR_EDIT}?${queryParams}`,
    );
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const response = await deleteFromFiorentApi(
      `${DELETE_WORK_ORDERS_DELETE}?${queryParams}`,
      null,
    );
